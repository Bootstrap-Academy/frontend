<template>
  <section class="pb-4">
    ...
    <div v-for="(task, i) of tasks" :key="`task-${i}`" class="flex gap-card">
      <article
        class="grid w-full grid-cols-[minmax(0,1fr)_auto] gap-y-2 border-2 px-4 py-2 style-box mt-box gap-x-card-sm"
        :class="
    ...
    <Modal v-if="taskIndex > -1">
      <div class="card w-full max-w-screen-lg bg-secondary style-card">
