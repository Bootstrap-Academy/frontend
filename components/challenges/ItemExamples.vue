    ...
    <section v-for="(example, i) of exampleElements" :key="i" class="relative">
      ...
      >
        <div class="flex justify-between mb-4">
          ...
          <div
            v-if="example.solved == 'solved' && !example.loading"
        ...
        <div
          class="text-sm text-error bg-primary py-2 px-4 rounded-md my-4 flex items-center space-x-6"
        ...
        <div
          class="text-sm bg-primary py-2 px-4 rounded-md my-4"
          v-if="!!example?.stderr && !example.loading"
        >
...
function successHandler(success: any, id: any) {
  ...
  if (!!success.compile && (success.compile?.stdout != '' || success.compile?.stderr != '')) {
    exampleElements.value[atIndex].stderr = success.compile?.stderr ?? '';
    exampleElements.value[atIndex].stdout = success.compile?.stdout ?? '';
  } else if (!!success.run && (success.run?.stdout != '' || success.run?.stderr != '')) {
    exampleElements.value[atIndex].stderr = success.run?.stderr ?? '';
    exampleElements.value[atIndex].stdout = success.run?.stdout ?? '';
  }
...
function setResonBasedOnVerdict(success: any, id: any) {
  ...
  exampleElements.value[atIndex].solved = verdictMapping[success?.verdict] || null;
}
