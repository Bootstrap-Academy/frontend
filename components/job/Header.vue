<template>
  <section class="relative h-fit rounded bg-secondary shadow-xl md:col-span-2">
    <img
      ...
      class="h-52 w-full object-cover brightness-50"
    />

    <article class="card bg-secondary style-card">
      <div class="flex flex-wrap-reverse items-center justify-between gap-card">
        <p class="text-subheading-1 mb-0 flex items-center text-accent gap-box">
          <img
            :src="companyImage"
            :alt="t('AltAttributes.CompanyLogo')"
            class="h-7 w-7 rounded-3xl object-cover"
          />
          {{ companyName }}
        </p>
        <p class="text-body-2 text-right text-subheading">{{ lastUpdated }}</p>
      </div>

      <h1 class="text-heading-1 mt-box mb-card-sm">{{ jobTitle }}</h1>

      <div class="flex flex-wrap items-center gap-card">
    ...
    <Btn class="absolute p-0 top-card right-card" @click="applyForJob">
      {{ t("Buttons.ApplyNow") }}
    </Btn>
  </section>
</template>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const companyImage = computed(() => {
      return props.data?.company?.logo_url ?? `/images/about-${getRandomNumber(1, 5)}.webp`;
    });
