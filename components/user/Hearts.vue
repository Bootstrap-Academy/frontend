<template>
  <div class="flex items-center gap-2 group" @click="gotoSubscription()">
    <div v-if="!isPremmium" class="text-heading hover:text-white">
      <article class="flex items-center gap-1 bg-tertiary px-5 py-2 h-10 rounded-full">
        <!-- <HeartIcon
        ...
        <PlusIcon class="flex-shrink-0 text-headiacang block h-3 w-3 sm:w-3.5 sm:h-3.5 ml-1" />
      </article>
    </div>
    <div
