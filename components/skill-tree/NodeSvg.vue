<template>
  ...
  >
    <g class="group" role="button" @click="onclickToggleActive">
      ...
      <g id="icon-box" class="cursor-pointer">
        <foreignObject
          ...
          :width="iconSize"
          :height="iconSize"
          class="origin-center scale-[1.4] animate-pulse rounded-full bg-[#451b80]"
        >
        ...
        <foreignObject
          ...
          :width="iconSize"
          :height="iconSize"
          class="rounded-full border-8 border-dotted border-[#f0a000]"
        >
          <div :style="{ width: iconSize + 'px', height: iconSize + 'px' }"></div>
        </foreignObject>
        ...
        <foreignObject
          ...
          :width="iconSize"
          :height="iconSize"
          class="rounded-full border-8 border-[#f0a000]"
        >
          <div :style="{ width: iconSize + 'px', height: iconSize + 'px' }"></div>
        </foreignObject>
      </g>
    ...
    >
      <g
        class="group/bookmark bookmarker origin-center cursor-pointer"
        @click.stop="onClickToggleBookmark"
      >
        <circle cx="62.5" cy="62.5" r="57.6" class="fill-dark" />
        <SvgStar
          v-if="isAuth"
          class="origin-center fill-secondary transition-all duration-300 ease-in-out group-hover/bookmark:scale-90"
          :class="isBookmarked ? 'fill-star' : ''"
        />
        <SvgStarOutline
          v-else
          class="origin-center text-body transition-all duration-300 ease-in-out group-hover/bookmark:scale-90"
        />
      </g>
    </svg>
  </svg>
  ...
  setup(props, { emit }) {
    ...
    const dashed = computed(() => {
      return {
        ...
        scale: props.active || props.completed ? "scale-100 opacity-100" : "scale-0 opacity-0",
      };
    });
