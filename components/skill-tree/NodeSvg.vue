<template>
  <svg viewBox="0 0 301 301" :width="size" :height="size" fill="none" xmlns="http://www.w3.org/2000/svg" tabindex="0"
    class="relative outline-none" :class="navigate ? 'cursor-pointer' : ''">
    <g class="group" role="button" @click="onclickToggleActive">
      <path id="dashed"
        ...
        fill="var(--dashed-color)" class="transformation group-focus:scale-100 group-focus:opacity-100"
        :class="dashed.scale" />
      ...
      <path v-if="outline.show" id="outline"
        ...
        fill="var(--outline-color)" class="transformation group-focus:scale-150" :class="outline.scale" />

      <g id="half" v-if="half.show"
        class="transformation group-focus:rotate-[225deg] group-focus:group-hover:rotate-[225deg]" :class="half.rotate">
        <path
    ...
    <svg x="180" y="200" width="100" height="100" version="1.1" viewBox="0 0 125 125"
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const normal = computed(() => {
      return !(props.disabled || props.active || props.completed || props.failed);
    });
    ...
    const dashed = computed(() => {
      return {
        ...
        scale:
          props.active || props.completed ? "scale-100 opacity-100" : "scale-0 opacity-0",
      };
    });
    ...
    const half = computed(() => {
      ...
      else if (props.active) rotate = "rotate-[135deg] group-hover:rotate-[135deg]";
      else if (props.completed) rotate = "rotate-[225deg] group-hover:rotate-[225deg]";
