<template>
  ...
  >
    ...
    <article class="mt-card">
      ...
      <button
        type="button"
        class="mr-7 inline-block min-w-[125px] cursor-pointer border-4 border-accent px-4 py-3 text-xl font-heading transition-basic style-box"
        :class="!business ? 'scale-105 bg-accent text-black' : 'text-white'"
      ...
      <button
        type="button"
        class="inline-block min-w-[125px] cursor-pointer border-4 border-accent px-4 py-3 text-xl font-heading transition-basic style-box"
        :class="business ? 'scale-105 bg-accent text-black' : 'text-white'"
        @click="business = true"
      >
        {{ t("Headings.Business") }}
  ...
  setup(props) {
    ...
    const form = reactive<IForm>({
      ...
      tags: {
        ...
        rules: [
          (v: string) =>
            !v || !form.tags.value || form.tags.value.length < 7 || "Error.InputMaxTags_7",
        ],
      },
    ...
    function errorHandler(res: any) {
      ...
      msg =
        msg == "Error.PermissionDenied" ? `${t("Inputs.Nickname")}: ${hintNickname.value}` : msg;
