    ...
    <div class="max-h-3xl flex h-full flex-col space-y-4">
      ...
      <div>
        <p class="flex flex-wrap items-center space-x-2">
          ...
          <RouterLink
            v-if="parentSkill?.id"
            :to="`/skill-tree/${parentSkill.id}`"
            class="font-bold text-accent"
            >{{ parentSkillName }}</RouterLink
          >
          <span v-if="subSkill?.id && parentSkill?.id">></span>
          <RouterLink
            v-if="subSkill?.id && parentSkill?.id"
            :to="`/skill-tree/${parentSkill.id}/${subSkill.id}`"
            class="font-bold text-accent"
            >{{ subSkillName }}</RouterLink
          >
        </p>
...
const parentSkill = computed(() => {
  return skillTree.value.skills.find((skill) => skill.skills.includes(props.event.skill_id));
});
const subSkill = computed(() => {
  return subTree.value.skills.find((skill) => skill.id == props.event.skill_id);
});
