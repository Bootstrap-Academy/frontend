...
export async function getEvaluator(taskId: string, subTaskId: any) {
  try {
    const response = await GET(
      `/challenges/tasks/${taskId}/coding_challenges/${subTaskId}/evaluator`
    );
    return [response, null];
  } catch (error: any) {
    return [null, error.data];
...
export async function getSolution(taskId: string, subTaskId: any) {
  try {
    const response = await GET(
      `/challenges/tasks/${taskId}/coding_challenges/${subTaskId}/solution`
    );
    return [response, null];
  } catch (error: any) {
    return [null, error.data];
  ...
  try {
    ...
    switch (verdict) {
      ...
      case "INVALID_OUTPUT_FORMAT":
        return [
          null,
          { error: "Error.Verdict.INVALID_OUTPUT_FORMAT", details: error?.data?.details },
        ];
        break;
      case "MEMORY_LIMIT_EXCEEDED":
        return [
          null,
          { error: "Error.Verdict.MEMORY_LIMIT_EXCEEDED", details: error?.data?.details },
        ];
        break;
      case "NO_OUTPUT":
      ...
      case "TIME_LIMIT_EXCEEDED":
        return [
          null,
          { error: "Error.Verdict.TIME_LIMIT_EXCEEDED", details: error?.data?.details },
        ];
        break;
      case "WRONG_ANSWER":
...
export async function updateCodingChallenge(taskId: string, subTaskId: any, body: any) {
  try {
    const response = await PATCH(
      `/challenges/tasks/${taskId}/coding_challenges/${subTaskId}`,
      body
    );
    const route = useRoute();
    const containQuizWord = route.fullPath.includes("/quizzes/");
