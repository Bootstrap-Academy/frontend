...
async function intercept({ request, requestId }) {
  ...
  if (url.pathname.startsWith("/shop/")) {
    ...
    if (url.pathname.includes('/documents/')) {
      ...
      return cmd('Fetch.fulfillRequest',{requestId,responseCode:response.status,responseHeaders:[{name:'Content-Type',value:response.headers.get('content-type') ?? 'application/octet-stream'},{name:'Access-Control-Allow-Origin',value:'*'}],body:bytes.toString('base64')});
    }
...
async function pendingUI(oid) {
  ...
  assert(
    !(await ev(`!!document.querySelector('#synthetic-paypal, [data-purchase-acceptance]')`))
  );
...
try {
  await fs.rm('/tmp/l1-browser-downloads',{recursive:true,force:true});
  await cmd('Browser.setDownloadBehavior',{behavior:'allow',downloadPath:'/tmp/l1-browser-downloads'});
  await navigate('/orders',true);
  await until(`document.querySelectorAll('article').length > 0`);
  const count=await ev(`document.querySelectorAll('article').length`);
  assert(count>=1);
  const article=`[...document.querySelectorAll('article')].find(a=>a.innerText.includes('Bereitstellungsnachweis'))`;
  await until(article);
  const id=await ev(`${article}.querySelector('p').innerText`);
  for(const label of ['Vereinbarte AGB','Widerrufsinformationen','Vertragsbestätigung','Bereitstellungsnachweis']) {
    await click(`[...${article}.querySelectorAll('button')].find(b=>b.innerText.includes(${JSON.stringify(label)}))`);
  }
  await pause(1000);
  const downloaded=await fs.readdir('/tmp/l1-browser-downloads');
  assert(downloaded.filter(f=>f.startsWith(id)).length===4,downloaded.join(','));
  for(const kind of ['terms','withdrawal','confirmation','fulfillment']) {
    const ext=['terms','withdrawal'].includes(kind)?'pdf':'txt';
    const bytes=await fs.readFile(`/tmp/l1-browser-downloads/${id}-${kind}.${ext}`);
    const expected=Buffer.from(await (await fetch(`${credentials.base}/shop/purchases/${id}/documents/${kind}`,{headers:{Authorization:'Bearer '+credentials.token}})).arrayBuffer());
    assert(bytes.equals(expected),kind+' bytes differ');
    if(ext==='pdf')assert(bytes.subarray(0,4).toString()==='%PDF');
    const unauthorized=await fetch(`${credentials.base}/shop/purchases/${id}/documents/${kind}`,{headers:{Authorization:'Bearer '+credentials.other}});
    assert.equal(unauthorized.status,404);
  }
  await screenshot('authenticated-orders');
  await cmd('Page.navigate',{url:app+'/vertrag-kuendigen'});
  await until(`document.body?.innerText.includes('Kündigung')`);
  assert(!(await ev(`!!document.querySelector('[data-paypal-recovery]')`)));
  assert.deepEqual(exceptions,[]);
  console.log('PASS authenticated order page; four actual downloaded artifacts byte-identical; cross-user 404; public cancellation unobstructed');
} finally {
  if(evidence)await fs.writeFile(`${evidence}/orders-requests.json`,JSON.stringify({requests,exceptions},null,2));
  ws.close();
}
