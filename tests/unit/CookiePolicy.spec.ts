...
;(globalThis as any).useRuntimeConfig = () => ({ public: { Gleap_API_KEY: 'test' } })
;(globalThis as any).useCookie = () => ({ value: false })         // Cookie=false für diesen Test
;(globalThis as any).useRoute  = () => ({ name: 'home' })         // Route global 
...
it('Klick auf Button setzt Cookie=true und blendet Dialog aus', async () => {
  ...
  ;(globalThis as any).useCookie = () => cookieRef
  ;(globalThis as any).useRoute  = () => ({ name: 'home' })
...
it('blendet den Dialog auf der Datenschutz-Seite aus', () => {
  const cookieRef = ref(false)
  ;(globalThis as any).useCookie = () => cookieRef
  ;(globalThis as any).useRoute  = () => ({ name: 'docs-privacy' })
